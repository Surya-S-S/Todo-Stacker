...
function Tasks({status,variant,transitions}) {
    ...
    phrase.set("TODO","to do");
    phrase.set("DOING","in progress");
    phrase.set("DONE","completed");

    useEffect(()=>{
        const url = BASE_URL+"/getTasksByStatus/"+userId+"/"+status;
    ...
    if(isEmpty) {
        return (
            <Container fluid="xl" style={{color:"white",textAlign:"center"}}>
                ...
                <h6>You have no tasks {phrase.get(status)}. Add some tasks</h6><br/>
            </Container>
        );
    }
